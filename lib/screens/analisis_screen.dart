...
class AnalisisScreen extends ConsumerWidget {
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                'Posisi Kas: ${data.selisih >= 0 ? 'Surplus' : 'Defisit'} ${_formatCurrency(data.selisih)}',
                              ),
                              if (data.sumberUtama != null)
                                Text(
                                  'Sumber Pemasukan Utama: ${data.sumberUtama}',
                                ),
                              if (data.posTerbesar != null)
                                Text(
                                  'Pos Pengeluaran Terbesar: ${data.posTerbesar}',
                                ),
                            ],
                          ),
                        ),
                    ...
                    if (data.kategoriPemasukan.isNotEmpty ||
                        data.kategoriPengeluaran.isNotEmpty) ...[
                      const Text(
                        'Rincian Kategori',
                        style: TextStyle(
                          fontSize: 20,
                    ...
                    if (data.riwayatBulanan != null &&
                        data.riwayatBulanan!.isNotEmpty) ...[
                      const Text(
  ...
  Future<void> _selectRange(BuildContext context, WidgetRef ref) async {
    ...
    if (picked != null) {
      ref.read(analisisFilterProvider.notifier).state = AnalisisFilter(
        start: picked.start,
        end: picked.end,
      );
      ref.invalidate(analisisProvider);
  ...
  Future<void> _generateDummy(BuildContext context, WidgetRef ref) async {
    final confirm = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: const Text(
              'Generate data dummy untuk beberapa bulan ke belakang? (Abaikan jika sudah ada data).',
            ),
            actions: [
              TextButton(
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Menyiapkan dummy data...')));
    try {
      await DatabaseHelper.instance.seedDummyData(
        monthsBack: 6,
        maxTransaksiPerBulan: 28,
      );
      if (context.mounted) {
        ref.invalidate(analisisProvider);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Dummy data selesai'),
  ...
  List<Widget> _buildRiwayatBulananSections(List<dynamic> riwayat) {
    ...
    return entries.map((entry) {
      ...
      final pemasukan = entry.value
          .where((e) => e.jenis == 'Pemasukan')
          .fold<int>(0, (p, e) => p + (e.total as int));
      final pengeluaran = entry.value
      ...
      return ExpansionTile(
        ...
        subtitle: Text(
          'Pemasukan ${_formatCurrency(pemasukan)} · Pengeluaran ${_formatCurrency(pengeluaran)} · Selisih ${_formatCurrency(selisih)}',
          style: TextStyle(
            color: selisih >= 0 ? Colors.green : Colors.red,
            fontSize: 12,
          ),
          ...
          child: Column(
            ...
            children: [
              ...
              ...rb.items.map<Widget>(
                (item) => Row(
                  children: [
                    ...
                    Text(
                      '${item.persentaseDariJenis.toStringAsFixed(1)}%',
              ...
              Align(
                ...
                child: Text(
                  'Total ${rb.jenis}: ${_formatCurrency(rb.total)}',
                  style: TextStyle(fontWeight: FontWeight.bold, color: color),
                ),
              ),
            ],
